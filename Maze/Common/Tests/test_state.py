...
extra_tile = Tile('┐')
players = [
    Player(None, test_board[0][0], test_board[1][2], (0, 0)),
...
def test_active_on_goal_tile2():
    ...
    players = [
        ...
        Player(None, test_board[2][0], test_board[2][2], (2, 0)),
    ]
...
def test_shift1():
    ...
    s = State(players=test_players, board=board, extra_tile=extra_tile)
    s.shift(0, 1, True)
    assert s.get_players()[0].get_position() == (0, 1) and s.get_players()[1].get_position() == (0, 0), \
...
def test_shift2():
    ...
    s = State(players=test_players, board=board, extra_tile=extra_tile)
    s.shift(0, -1, True)
    assert s.get_players()[0].get_position() == (0, 2) and s.get_players()[1].get_position() == (0, 1), \
        'Shift row didn\'t work'
...
def test_shift3():
    ...
    s = State(players=test_players, board=board, extra_tile=extra_tile)
    s.shift(0, 1, False)
    assert s.get_players()[0].get_position() == (1, 0) and s.get_players()[1].get_position() == (0, 0), \
