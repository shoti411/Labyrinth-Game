...
def test_check_degrees1():
    re = riemann_strategy.check_degrees(Coordinate(0, 2), player, board, extra_tile, 2, 1, False)
    assert re == 0, 'Cannot find correct degree'

def test_check_degrees2():
    extra_tile = Tile('└')
    re = riemann_strategy.check_degrees(Coordinate(1, 1), player, board, extra_tile, 0, 1, True)
...
def test_check_direction1():
    re = riemann_strategy.check_direction(Coordinate(0, 2), player, board, extra_tile, 2, False)
    assert re == (0, 1), 'Cannot find correct degree and direction'

def test_check_direction2():
    re = riemann_strategy.check_direction(Coordinate(1, 1), player, board, extra_tile, 0, True)
    assert re == (90, 1), 'Cannot find correct degree and direction'

def test_check_direction3():
    re = riemann_strategy.check_direction(Coordinate(1, 1), player, board, extra_tile, 2, True)
    assert re == -1, 'Found degree and direction that doesnt exist'

def test_check_row_shift1():
    re = riemann_strategy.check_row_shift(Coordinate(1, 1), player, board, extra_tile)
    assert re == (90, 1, 0, True), 'Cannot find correct degree, direction, index and isrow'

def test_check_row_shift2():
    re = riemann_strategy.check_row_shift(Coordinate(2, 2), player, board, extra_tile)
    assert re == (-1), 'Found degree, direction, index and isrow when it doesnt exist'

def test_check_col_shift1():
    re = riemann_strategy.check_col_shift(Coordinate(0, 2), player, board, extra_tile)
...
def test_check_col_shift2():
    ...
    re = riemann_strategy.check_col_shift(Coordinate(1, 2), player, Board(board=test_board), extra_tile)
    assert re == -1, 'Found degree, direction, index and isrow when it doesnt exist'
