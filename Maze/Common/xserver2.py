...
def xserver(in_stream):
    ...
    ref = Referee(goals=json_to_goals_array(json_objects[0]))
    s = Server('localhost', int(sys.argv[1]), referee=ref, state=state)
...
def json_to_state(json_state):
    ...
    players = json_to_ref_players(json_state, board)
...
def json_to_ref_players(json_object, board):
