...
def xgame(in_stream):
    ...
    turn_data, goals = handle_json(json_objects)
    return return_output(turn_data, goals)
...
def return_output(turn_data, goals):
    ref = Referee(goals=goals)
...
def handle_json(json_objects):
    ...
    try:
        goals = json_to_goals_array(json_objects[1]['goals'], board)
    except IndexError:
        goals = []
    return s, goals
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        f_out = open(rf'C:\Users\Shaun\College\Fall 2022\Software Development\egoless-bears\10\Tests\{i}-out.json', 'w')
